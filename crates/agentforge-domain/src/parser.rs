...
///
/// - `## <n>. <Title>` — a section, where `<n>` is a plain integer. Content
...
/// Split a section heading `## <n>. <Title>` into `(id, title)`.
///
/// Section ids must be plain integers (e.g. `5`, `14`).
fn parse_section_heading(line: &str) -> Result<(RuleId, String), DomainError> {
  ...
  if id_raw.is_empty() || !id_raw.chars().all(|c| c.is_ascii_digit()) {
    return Err(DomainError::InvalidRuleId(format!(
      "section id must be an integer: {id_raw}"
    )));
  }
